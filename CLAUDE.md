...
**OWL/RDFS/SKOS infrastructure types are excluded.** Any `rdf:type` in the OWL (`owl:`), RDFS (`rdfs:`), RDF-syntax (`rdf:`), or SKOS core (`skos:`) namespaces is classified as a meta-type by `is_meta_type()` in `searchindex.py` and silently skipped — it is never looked up in the source RDF and requires no source-side metadata. Source systems may freely use property characteristics such as `owl:ObjectProperty`, `owl:FunctionalProperty`, or `rdfs:Class`, and may emit `skos:Concept` / `skos:ConceptScheme` subjects (e.g. local SKOS vocabularies), without supplying type metadata for them.
